...
server <- function(input, output, session) {
   ...
   observeEvent(input$search,{
     ...
     if (!crs_search()) {
      ...
      url<-paste0("https://api.maptiler.com/coordinates/search/",
                  input$country,
                  ".json?key=76tZpqR88DJrIw2yG2jG&deprecated=CRS-PROJCRS")
      req <- httr::GET(url)
  ...
  observeEvent(input$go,{
    ...
    leaflet() %>%
      ...
      addAwesomeMarkers(
        data = dt1_re(), lng = ~get(input$x), lat = ~get(input$y),
        label = ~get(input$id),
        icon = icons1,
        group = ~CLUSTER_ID
      ) %>% 
      addCircleMarkers(
        data = dt0_re(), lng = ~get(input$x), lat = ~get(input$y),
  ...
  generate_string <- function(name, values) {
    if (length(values) == 0) {
      return("")
    } else {
      return(paste0(name, ": ", paste(values, collapse = ", "), "; "))
    }
  }
  ...
  all_case<-reactive({
    db_re() %>% 
    mutate(CLUSTER_ID:=ifelse(is.na(CLUSTER_ID),0,CLUSTER_ID)) %>% 
    mutate(name=generate_string("name", input$name),
           desc=generate_string("desc", input$desc),
           cmt=generate_string("cmt", input$cmt),
           src=generate_string("src", input$src)) %>% 
    select(name,desc,cmt,src) })
  
  cluster_case<-reactive({
    st_as_sf(dt1_re(),coords = c(input$x,input$y),crs=4326) %>% 
      mutate(name=generate_string("name", input$name),
             desc=generate_string("desc", input$desc),
             cmt=generate_string("cmt", input$cmt),
             src=generate_string("src", input$src)) %>% 
